...
async function loadFriends() {
    ...
    const { data: followingData } = await SB.from("follows").select("*, profiles!following_id(*)").eq("follower_id", USER.id);
    const following = followingData?.map(f => f.profiles) || [];
    const followingIds = new Set(following.map(f => f.id));
    
    // Get users who follow you
    const { data: followersData } = await SB.from("follows").select("*, profiles!follower_id(*)").eq("following_id", USER.id);
    const followers = followersData?.map(f => f.profiles) || [];
